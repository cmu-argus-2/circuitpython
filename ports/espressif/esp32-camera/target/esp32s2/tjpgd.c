...
void *alloc_pool(       /* Pointer to allocated memory block (NULL:no memory available) */
    ...
    if (jd->sz_pool >= nd) {
        ...
        jd->pool = (void *)(rp + nd);           /* Allocate required bytes */
    }
...
JRESULT mcu_load(
    ...
    for (blk = 0; blk < nby + nbc; blk++) {
        ...
        if (b < 0) {
            return 0 - b;                                               /* Err: invalid code or input */
        }
        ...
        if (JD_USE_SCALE && jd->scale == 3) {
            *bp = (*tmp / 256) + 128;                   /* If scale ratio is 1/8, IDCT can be omitted and only DC element is used */
        } else {
...
JRESULT restart(
    JDEC *jd,           /* Pointer to the decompressor object */
    WORD rstn           /* Expected restert sequence number */
    ) {
    UINT i, dc;
    WORD d;
    BYTE *dp;
...
JRESULT jd_prepare(
    ...
    UINT (*infunc)(JDEC *, BYTE *, UINT),       /* JPEG stream input function */
